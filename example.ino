...
StripWrapper wrapper(ROW_SIZE, COLUMN_SIZE, DATA_PIN_A, CLOCK_PIN_A);
...
{
  for (uint8_t y=0; y < panel.column_size(); y++)
  {
    for (uint8_t x=0; x < panel.row_size() ; x++)
    {
...
void knight_rider()
{
  ...
  {
    reverse = (i%2);
    for(uint8_t j=0; j < panel.column_size() / 2; j++)
    {
      uint8_t p;
      if (reverse)
      {
        p = panel.column_size() - (2 * j);
      }
      else
      {
        p = 2 * j;
...
void fill_rainbow()
{
  ...
  for(uint8_t i=panel.column_size(); i > 0; i--)
  {
...
void multi_rainbow()
{
  for(uint8_t r=0; r < panel.column_size() / 2; r++)
  {
